...
export const studyRoomStatusEnum = pgEnum('study_room_status', ['active', 'inactive']);

// Study room type enum  
...
export const studyRooms = pgTable("study_rooms", {
  ...
  roomType: studyRoomTypeEnum("room_type").notNull().default('public'),
  ...
  return {
    hostIdIdx: index("idx_study_rooms_host_id").on(table.hostId),
    roomCodeIdx: index("idx_study_rooms_room_code").on(table.roomCode),
    statusIdx: index("idx_study_rooms_status").on(table.status),
    typeIdx: index("idx_study_rooms_type").on(table.roomType),
  };
});

// Study room participant roles enum
...
export const studyRoomParticipants = pgTable("study_room_participants", {
  ...
  return {
    roomIdIdx: index("idx_study_room_participants_room_id").on(table.roomId),
    userIdIdx: index("idx_study_room_participants_user_id").on(table.userId),
    activeIdx: index("idx_study_room_participants_active").on(table.isActive),
  };
});
...
export const studyRoomMessages = pgTable("study_room_messages", {
  ...
  return {
    roomIdIdx: index("idx_study_room_messages_room_id").on(table.roomId),
    userIdIdx: index("idx_study_room_messages_user_id").on(table.userId),
    createdAtIdx: index("idx_study_room_messages_created_at").on(table.createdAt),
  };
});
...
export const whiteboardSessions = pgTable("whiteboard_sessions", {
  ...
  whiteboardData: jsonb("whiteboard_data"), // Canvas drawing data
  isActive: boolean("is_active").default(true),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  endedAt: timestamp("ended_at"),
}, (table) => {
  return {
    roomIdIdx: index("idx_whiteboard_sessions_room_id").on(table.roomId),
    hostIdIdx: index("idx_whiteboard_sessions_host_id").on(table.hostId),
    activeIdx: index("idx_whiteboard_sessions_active").on(table.isActive),
  };
});
...
export const screenSharingSessions = pgTable("screen_sharing_sessions", {
  ...
  return {
    roomIdIdx: index("idx_screen_sharing_sessions_room_id").on(table.roomId),
    userIdIdx: index("idx_screen_sharing_sessions_user_id").on(table.userId),
    activeIdx: index("idx_screen_sharing_sessions_active").on(table.isActive),
  };
});

// Insert schemas for study room tables
...
export const insertStudyRoomMessageSchema = createInsertSchema(studyRoomMessages)
  .omit({ id: true, createdAt: true });

export const insertWhiteboardSessionSchema = createInsertSchema(whiteboardSessions)
  .omit({ id: true, createdAt: true, endedAt: true });
